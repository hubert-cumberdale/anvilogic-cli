...
- Route model and contract JSON/YAML through one strict offline loader while preserving Markdown
  catalogs and schema-only catalogs. Keep aliases rejected, add precise remediation, and document
  an operator-controlled trusted YAML-to-JSON workflow.
- Move request validation and transport orchestration out of the CLI parser without changing
  commands, operation IDs, mutation gates, output formats, or response-file behavior.
- Harden OpenAPI 3.0 validation for recursive local references, composition, nullable values,
  request/response `readOnly` and `writeOnly` direction, formats, and constraints. Reject
  unsupported validation keywords instead of silently ignoring them.
- Add `adoption assess`, an offline aggregate-only harness for operator-controlled contracts and
  sanitized request/response fixtures. It has no transport or output-file capability and neither
  renders nor retains payloads.
- Record the v0.3.5 hardening milestone and a prioritized issue backlog without expanding endpoint
  families, mutation semantics, registry facts, distribution channels, or lifecycle claims.

## 0.3.4 - 2026-09-23

- Evolve the reviewed operation registry to v4 while preserving all 344 IDs and method totals.
  Record 94 required bodies, 58 optional bodies, 12 conservative private-only unknowns, and 180
  bodyless operations from the unchanged checksum-pinned public source and an independently pinned
  v0.3.3 registry seed.
- Enforce named-invocation body presence: require known-required and private-unknown bodies, allow
  documented optional bodies to be omitted, and reject bodies for bodyless operations.
- Add opt-in `invoke --validate-request` and `--validate-response` using a caller-controlled full
  OpenAPI 3.0.x JSON/YAML document. Resolve exact method/path contracts, support inline and local
  JSON Pointer schemas, reject external references, and never fetch at runtime.
- Validate requests before transport and select response schemas by exact status, status family,
  then `default`. Keep validation silent on success, preserve HTTP exit 5 for valid documented
  errors, and retain mutation apply gates and non-retry behavior.
