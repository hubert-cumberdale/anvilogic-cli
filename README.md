...
Current release: `0.3.3` (Alpha; not production-qualified).
...
date. It contains all 317 operations in the checksum-pinned public OpenAPI 3.0 document plus 27
...
bound to a model; use `call` with an explicit local catalog when model validation is needed.
...
```bash
git show v0.3.2:src/anvilogic_cli/operations.json \
  > /secure/path/operations-v0.3.2.json
python3 scripts/generate_operations.py \
  --input /secure/path/anvilogic-combined-apis.yaml \
  --existing /secure/path/operations-v0.3.2.json \
  --check
```

The generator requires that exact checksum-pinned `v0.3.2` registry as an independent seed; it
