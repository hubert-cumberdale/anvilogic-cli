...
- `operations.py` validates the bundled v3 mixed-evidence fact registry, resolves path parameters,
  and restricts
...
- `generate_operations.py` verifies the exact reviewed public OpenAPI bytes and the independently
  pinned `v0.3.2` private-fact seed, derives collision-free IDs, and emits no schemas.
...
entries have no model bindings. Every POST, PUT, PATCH, and DELETE remains apply-gated and
non-retried because documentation or search-like behavior does not prove absence of effects.
