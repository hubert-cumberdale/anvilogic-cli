...
- `schemas.py` safely loads only caller-supplied Markdown, JSON, YAML, or YML files and provides
  search, reference expansion, samples, and JSON Schema validation. It performs no network fetch.
- `contracts.py` safely loads a full local OpenAPI 3.0.x JSON/YAML document, resolves operations by
  exact normalized method/path, permits only local JSON Pointer references, and validates selected
  request and response JSON without runtime fetching.
