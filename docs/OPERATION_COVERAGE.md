...
The v4 registry contains 344 fact entries: 317 from the public OpenAPI document plus 27
...
| --- | --- | --- | --- | --- | --- |
| All 344 registry facts | Yes | Generic `invoke` resolution | Metadata, uniqueness, path, query, body presence/requiredness, totals, and ID generation | Registry and provenance docs | Yes |
...
| 344 local operation contracts | Operator supplied | Opt-in `invoke` request/response validation | Exact matching, recursive local references, composition, nullable and directional properties, constraints, status selection, malformed and adversarial inputs | OpenAPI 3.0.x required | No schemas bundled |
| Sanitized adoption fixtures | Operator supplied | Offline aggregate-only assessment | Positive, negative, unusable, no-network, no-retention, and no-payload-diagnostic cases | Versioned local fixture format | Outcomes only |
...
- Required or private-unknown bodies must be supplied, optional bodies may be omitted, and
  bodyless operations reject bodies.
