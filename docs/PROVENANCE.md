...
This ledger describes every source class represented in the public `v0.3.5` tree. Facts are
...
| Operator-supplied adoption fixtures | User-controlled, sanitized local input; sensitivity remains the operator's responsibility | Assessed in memory only through explicit local paths | Aggregate validation outcome counts only; no documents, payloads, or per-operation results retained |
