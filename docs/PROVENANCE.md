...
This ledger describes every source class represented in the public `v0.3.3` tree. Facts are
...
| Public Anvilogic OpenAPI 3.0 document | Publicly accessible at the recorded GitBook URL; no explicit redistribution license was found, and copyright remains with its publisher | Checksum-pinned factual distillation and attribution only | Method, normalized path, query names, request-body presence, source metadata, and verification date; no YAML or schemas |
