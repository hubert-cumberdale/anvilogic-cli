...
This ledger describes every source class represented in the public `v0.3.2` tree. Facts are
...
| Reviewed private traffic evidence | Private evidence; raw messages and tenant material are not redistributed | Facts were distilled through human review | Operation ID, method, normalized path, query names, body presence, and review metadata only |
...
| Public Anvilogic documentation | Publicly accessible; copyright remains with its publisher | Facts summarized without verbatim republication | Product/authentication context only |
...
- A concrete method or path is marked `confirmed` only when directly present in reviewed evidence.
  Confidence does not convert evidence into an official API contract.
