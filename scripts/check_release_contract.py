...
OPERATIONS_PATH = ROOT / "src" / "anvilogic_cli" / "operations.json"
OPERATIONS_SHA256 = "f870d50fd47ad8859a1be1146357334391791ab9da434ccd0b115d344b3e662a"
PUBLIC_OPERATION_COUNTS = {"DELETE": 18, "GET": 140, "PATCH": 17, "POST": 132, "PUT": 10}
UNION_OPERATION_COUNTS = {"DELETE": 18, "GET": 155, "PATCH": 17, "POST": 144, "PUT": 10}
...
def check_operation_registry() -> list[str]:
    errors: list[str] = []
    try:
        body = OPERATIONS_PATH.read_bytes()
        payload = json.loads(body)
        operations = payload["operations"]
        if not isinstance(operations, list):
            raise TypeError("operations is not an array")
    except (OSError, UnicodeDecodeError, json.JSONDecodeError, KeyError, TypeError) as exc:
        return [f"Bundled operation registry is invalid: {exc}"]
    digest = hashlib.sha256(body).hexdigest()
    if digest != OPERATIONS_SHA256:
        errors.append(
            f"Bundled operation registry SHA-256 is {digest}; expected {OPERATIONS_SHA256}."
        )
    public = [item for item in operations if "public-openapi" in item.get("evidence", [])]
    private = [
        item for item in operations if "distilled-private-traffic" in item.get("evidence", [])
    ]
    public_counts = dict(sorted(Counter(item.get("method") for item in public).items()))
    union_counts = dict(sorted(Counter(item.get("method") for item in operations).items()))
    requiredness = Counter(item.get("request_body_required") for item in operations)
    if payload.get("version") != 4:
        errors.append("Operation registry must use version 4.")
    if len(public) != 317 or public_counts != PUBLIC_OPERATION_COUNTS:
        errors.append(
            f"Public operation facts are {len(public)} with {public_counts}; expected 317 "
            f"with {PUBLIC_OPERATION_COUNTS}."
        )
    if len(private) != 74:
        errors.append(f"Private-evidence operation facts are {len(private)}; expected 74.")
    if len(operations) != 344 or union_counts != UNION_OPERATION_COUNTS:
        errors.append(
            f"Operation union is {len(operations)} with {union_counts}; expected 344 "
            f"with {UNION_OPERATION_COUNTS}."
        )
    expected_requiredness = Counter({False: 238, True: 94, None: 12})
    if requiredness != expected_requiredness:
        errors.append(
            f"Operation body-required totals are {dict(requiredness)}; expected "
            f"{dict(expected_requiredness)}."
        )
    try:
        packaged = resources.files("anvilogic_cli").joinpath("operations.json").read_bytes()
    except OSError as exc:
        errors.append(f"Installed operation registry is unavailable: {exc}")
    else:
        if packaged != body:
            errors.append("Installed and source operation registries differ.")
    return errors
...
def check_public_boundary() -> list[str]:
    ...
    forbidden = [
        ROOT / "models.md",
        ROOT / "openapi.yaml",
        ROOT / "anvilogic-combined-apis.yaml",
        ROOT / "src" / "anvilogic_cli" / "schemas.json",
    ]
...
CHECKS = {
    ...
    "operation-registry": check_operation_registry,
