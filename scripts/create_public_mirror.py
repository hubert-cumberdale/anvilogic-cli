...
def ensure_release_source(ref: str, authority_ref: str, *, root: Path = ROOT) -> str:
    """Require a lightweight release tag at the private authority branch tip."""
    tag_ref = f"refs/tags/{ref}"
    object_type = _git_output(["git", "cat-file", "-t", tag_ref], root=root)
    if object_type != "commit":
        raise RuntimeError(f"strict export requires a lightweight tag, got {object_type}")
    source_commit = resolve_commit(tag_ref, root=root)
    authority_commit = resolve_commit(authority_ref, root=root)
    if source_commit != authority_commit:
        raise RuntimeError(
            f"release tag {ref} is {source_commit}, but authority ref "
            f"{authority_ref} is {authority_commit}"
        )
...
def prepare_public_mirror(
    *,
    ...
    if not allow_dirty:
        ...
        source_commit = ensure_release_source(ref, authority_ref, root=root)
    else:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--authority-ref",
        default="master",
        help="Private authority branch tip that the lightweight release tag must match.",
    )
