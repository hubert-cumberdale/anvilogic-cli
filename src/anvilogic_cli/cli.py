...
from anvilogic_cli.cli_adoption import adoption_app
from anvilogic_cli.cli_threat_scenarios import threat_scenarios_app
from anvilogic_cli.client import AnvilogicClient, redacted_plan
...
from anvilogic_cli.invocation import (
    InvocationRequest,
...
def _execute_call(
    *,
    ...
    will_send = will_send_request(normalized_method, dry_run=dry_run, apply=apply)
    ...
    if request_model and data is None and data_file is None:
        raise typer.BadParameter("--request-model requires --data or --data-file.")
    verify_tls = config.verify_tls and not insecure
    before_send = (
        _warn_unverified_transport
        if will_send and (resolved_base_url.startswith("http://") or not verify_tls)
        else None
    )
    try:
        outcome = execute_invocation(
            InvocationRequest(
    ...
    if outcome.response is None:
        _emit_json(redacted_plan(outcome.plan))
        if outcome.mutation_preview:
...
def _warn_unverified_transport() -> None:
