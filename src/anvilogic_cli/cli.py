...
def main(
    ...
    models_path: Path | None = typer.Option(
        ...
        exists=True,
        readable=True,
        help="Caller-supplied Markdown model export or OpenAPI JSON document.",
    ),
) -> None:
...
def invoke_operation(
    ctx: typer.Context,
    operation_id: str = typer.Argument(help="Observed operation ID."),
    ...
    apply: bool = typer.Option(
        False, help="Send POST operations; otherwise only preview the request."
    ),
    ...
    try:
        ...
        _abort(exc, ExitCode.USAGE)
    if operation.request_body and data is None and data_file is None:
