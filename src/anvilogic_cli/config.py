...
def effective_models_path(override: Path | str | None = None) -> Path | None:
    value = override or os.getenv(ENV_MODELS_PATH)
    if not value:
        return None
    rendered = str(value).strip()
    if URI_SCHEME_RE.match(rendered) and not WINDOWS_DRIVE_RE.match(rendered):
        raise ConfigError("Models path must be a local file, not a URI.")
