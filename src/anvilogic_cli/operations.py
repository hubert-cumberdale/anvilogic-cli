    ...
    def stats(self) -> dict[str, Any]:
        return {
            ...
            "request_body_required": {
                "false": sum(item.request_body_required is False for item in self._operations),
                "true": sum(item.request_body_required is True for item in self._operations),
                "unknown": sum(item.request_body_required is None for item in self._operations),
            },
            "source": self.source,
        }
...
def _parse_operation(raw: object) -> Operation:
    ...
    request_body_required = raw.get("request_body_required")
    risk = raw.get("risk")
    ...
    if request_body_required is not None and not isinstance(request_body_required, bool):
        raise OperationCatalogError(
            f"Operation '{operation_id}' has invalid body-required metadata."
        )
    if not request_body and request_body_required is not False:
        raise OperationCatalogError(
            f"Operation '{operation_id}' has inconsistent body-required metadata."
        )
    if request_body_required is None and raw.get("evidence") != ["distilled-private-traffic"]:
        raise OperationCatalogError(
            f"Operation '{operation_id}' has unjustified unknown body requiredness."
        )
