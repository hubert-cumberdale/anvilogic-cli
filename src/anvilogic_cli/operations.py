...
VALID_EVIDENCE_CLASSES = {"distilled-private-traffic"}
...
class OperationCatalog:
    ...
    def stats(self) -> dict[str, Any]:
        return {
            ...
            "evidence_classes": _count_values(
                item.evidence_class for item in self._operations
            ),
            "source": self.source,
        }
...
def _validate_source(source: dict[str, Any]) -> None:
    expected = {
        "kind": "distilled_fact_registry",
        "evidence_provenance": "private traffic",
        "contract_status": "provisional_not_official",
...
def _parse_operation(raw: object) -> Operation:
    ...
    if evidence_class not in VALID_EVIDENCE_CLASSES:
        raise OperationCatalogError(f"Operation '{operation_id}' has invalid evidence class.")
