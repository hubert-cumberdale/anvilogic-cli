...
from anvilogic_cli.errors import TransportError
...
def test_version() -> None:
    ...
    assert result.stdout.strip() == "anvilogic-cli 0.3.5"
...
def test_unverified_transport_warning_precedes_network_failure(monkeypatch) -> None:
    def failed_send(*_args, **_kwargs):
        raise TransportError("synthetic transport failure")

    monkeypatch.setattr("anvilogic_cli.cli.AnvilogicClient.send", failed_send)
    result = runner.invoke(
        app,
        [
            "call",
    ...
    assert "request transport is not fully verified" in result.stderr
