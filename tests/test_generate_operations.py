        ...
        "operations": [
            {
                ...
                "evidence": ["distilled-private-traffic"],
                "id": "stable.get.thing",
...
def test_generation_is_deterministic_and_preserves_reviewed_ids() -> None:
    ...
    assert operations["stable.get.thing"]["request_body"] is True
    assert operations["stable.get.thing"]["request_body_required"] is False
    assert operations["delete.new-items.by-item-id"]["risk"] == "destructive"
    assert operations["private.only"]["confidence"] == "confirmed"
    assert operations["private.only"]["request_body_required"] is None
...
def test_generation_rejects_derived_id_collisions() -> None:
    ...
    with pytest.raises(ValueError, match="collision"):
        generate_operations.build_registry(collision, {"version": 3, "operations": []})
