...
def _source_repository(tmp_path: Path) -> Path:
    ...
    _git(["init"], root)
    _git(["symbolic-ref", "HEAD", "refs/heads/master"], root)
    _git(["add", "-A"], root)
    subprocess.run(
        [
            "git",
...
def test_public_mirror_rejects_annotated_release_tag(tmp_path: Path) -> None:
    source = _source_repository(tmp_path)
    _git(["tag", "-d", "v1.2.3"], source)
    _git(
        [
            "-c",
            "user.name=Test",
    ...
    with pytest.raises(RuntimeError, match="lightweight tag"):
        create_public_mirror.prepare_public_mirror(
            ref="v1.2.3",
            output_dir=tmp_path / "public",
            scan_package=False,
...
def test_public_mirror_rejects_tag_not_at_authority_tip(tmp_path: Path) -> None:
    source = _source_repository(tmp_path)
    (source / "README.md").write_text("# New authority tip\n", encoding="utf-8")
    ...
    with pytest.raises(RuntimeError, match="authority ref master"):
        create_public_mirror.prepare_public_mirror(
            ref="v1.2.3",
            output_dir=tmp_path / "public",
            scan_package=False,
